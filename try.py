...
root = Tk()
root.title("Contact Management System Created By Meenakshi Papnai")
root.geometry("780x400+0+0")
#root.resizable(90, 90)
root.config(bg="pink")
...
def Update():
    if gender.get() == "":
        result = tkMessageBox.showwarning('', 'Please Double-Click on the Field', icon="warning")
    else:
        tree.delete(*tree.get_children())
    conn = sqlite3.connect("contactdb.db")
    cursor = conn.cursor()
    ...
    gender1 = gender.get()
    if not gender1:
        tkMessageBox.showerror("Select gender")
...
def UpdateContactWindow(event):
   ...
   # UpdateWindow.config(bg="dark gray")
    ...
    Transgender = Radiobutton(RadioGroup, text="Other", variable=gender, value="Other", font=('arial', 14)).pack(side=LEFT)
# LABELS
    lbl_title = Label(FormTitle, text="Updating Contacts", font=('arial', 16), bg="lightgreen", width=300)
...
# ===================FRAMES==============================
    ...
    Male = Radiobutton(RadioGroup, text="Male", variable=gender, value="Male", font=('arial', 14)).pack(side=LEFT)
    Female = Radiobutton(RadioGroup, text="Female", variable=gender, value="Female", font=('arial', 14)).pack(side=LEFT)
    Transgender = Radiobutton(RadioGroup, text="Others", variable=gender, value="Others", font=('arial', 14)).pack(side=LEFT)
# ===================LABELS==============================
    lbl_title = Label(FormTitle, text="Adding New Contacts", bd=12, relief=GROOVE, fg="White", bg="lightgreen",
...
Top.pack(side=TOP)
Mid = Frame(root, width=650, bg="pink")
Mid.pack(side=BOTTOM)
f1 = Frame(root, width=410, height=8, bd=10, bg="lightgreen")
f1.pack(side=BOTTOM)
flb = Frame(f1, width=7, height=8, bd=10, bg="red")
flb.pack(side=BOTTOM)
MidLeft = Frame(Mid, width=500)
MidLeft.pack(side=LEFT, pady=10)
MidLeftPadding = Frame(Mid, width=370, bg="pink")
MidLeftPadding.pack(side=LEFT)
MidRight = Frame(Mid, width=500)
MidRight.pack(side=RIGHT, pady=10)
TableMargin = Frame(root, width=800)
TableMargin.pack(side=TOP)

# LABELS
lbl_title = Label(Top, text="Contact Management System", bd=12, relief=GROOVE, fg="black", bg="lightgreen",
width=100, font=("Calibri", 36, "bold"), pady=3)
lbl_title.pack(fill=X)


# BUTTONS
ButtonAdd = Button(flb, text='Add New Contact', bd=10, font=('arial', 12, 'bold'), relief="groove", fg="black",
bg="lightgreen", command=AddNewContact).grid(row=0, column=0, ipadx=20, padx=30)

ButtonDelete = Button(flb, text='Delete', bd=10, font=('arial', 12, 'bold'), relief="groove", command=Delete,
fg="black", bg="lightgreen").grid(row=0, column=1, ipadx=20)

ButtonUpdate = Button(flb, text='Update', bd=10, font=('arial', 12, 'bold'), relief="groove", command=Update,
fg="black", bg="lightgreen").grid(row=0, column=2, ipadx=20)

ButtonExit = Button(flb, text='Exit System', bd=10, font=('arial', 12, 'bold'), relief="groove", command=Exit,
fg="black", bg="lightgreen").grid(row=0, column=3, ipadx=20, padx=30)
